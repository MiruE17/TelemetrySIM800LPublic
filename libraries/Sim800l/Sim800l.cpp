...
void Sim800l::selectMemory(){
	SIM.println("AT+CPMS=\"ME\"");
	Serial.println(_readSerial());
}

bool Sim800l::regOperator(){
	SIM.println("AT+COPS=1,2,\"51010\"");
	_buffer=_readSerial();
...
String Sim800l::readAllSms(){
  SIM.print (F("AT+CMGF=1\r"));
	_buffer = "";
  if (( _readSerial().indexOf("ER")) ==-1) {
    SIM.print (F("AT+CMGL=ALL"));
    SIM.print("\r");
    _buffer=_readSerial();
